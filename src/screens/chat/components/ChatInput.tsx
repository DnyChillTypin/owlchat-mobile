  ...
  return (
    <View className="border-t border-border bg-card">
      ...
      <View className="p-2 pb-6 flex-row items-end space-x-2">
        ...
        <View className="flex-1 bg-muted rounded-3xl px-4 py-2 max-h-32">
          <TextInput
        ...
        <TouchableOpacity 
          className={`p-3 rounded-full ${content.trim() ? 'bg-primary' : 'bg-transparent'}`}
          onPress={content.trim() ? handleSend : () => onSendMessage("👍")}
        >
          {content.trim() ? (
