...
export const useChatMemberUser = () => {
  ...
  return {
    ...
    deleteChatMember,
    markAsRead: async (accountId: string | null, requesterId: string | null, chatId: string) => {
      setLoading(true);
      setError(null);
      try {
        return await chatMemberUserService.markAsRead(accountId, requesterId, chatId);
      } catch (err: any) {
        setError(err.message);
        throw err;
      } finally {
