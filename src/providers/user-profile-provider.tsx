...
export const UserProfileProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 2. Fetch exactly once on mount when authenticated
  useEffect(() => {
    if (isInitialMount.current && isAuthenticated) {
      console.log("[UserProfileProvider] Fetching initial profile...");
      refreshProfile();
      isInitialMount.current = false;
    } else if (isInitialMount.current && !isAuthenticated) {
      console.log("[UserProfileProvider] Not authenticated, skipping profile fetch");
      isInitialMount.current = false;
    }
